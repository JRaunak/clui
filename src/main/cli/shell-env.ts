 ...
 *
 * Approach: run the login shell, have it emit its env as JSON, and keep only
 * auth-relevant keys (by prefix). Curated-not-wholesale on purpose — blindly overlaying
...
/** Exact keys outside those prefixes worth carrying (region/provider togglers). */
const AUTH_EXACT = new Set(['AWS_REGION', 'AWS_DEFAULT_REGION', 'AWS_PROFILE', 'BEDROCK_REGION'])
/**
